...
import com.lihb.network.config.RetrofitsConfig
...
object RetrofitsManager {
    ...
    fun <T> getApiService(
        ...
        var iApi: Any? = serviceMap[clazz]
        if (iApi == null) {
            val key = "$baseUrl${clazz.simpleName}$tag"
            var retrofit: Retrofit? = retrofits[key]
            if (retrofit == null) {
